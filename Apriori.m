...
sup_min_percent = sup_min / transactionCount;
conf_min_percent = conf_min / transactionCount;
...
for itemsetIndex = 1:3
    ...
    else % List -> Candidate, using self-joining
    ...
    %% scan and count sup for each itemset
...
for k = 1:size(List{itemsetIndex, 1}, 1)
    ...
    for it = 1:itemsetIndex
        % head = the numbers before -> symbol
        % tail = the numbers after -> symbol
        union{it} = itemset;
        tail{it, 1} = union{it};
        ...
        head{it, 2} = tail{it, 1};
        tail{it, 2} = head{it, 1};
        % find the index of List that contains the corresponding itemset
